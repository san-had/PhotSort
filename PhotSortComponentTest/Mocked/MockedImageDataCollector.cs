...
namespace PhotSortComponentTest.Mocked
{
    public class MockedImageDataCollector : IImageDataCollector
    {
        public async Task<IList<IExifDataDto>> ReadExifDataOfFiles(string folderName)
        {
            ...
            var secondImageList = await GenerateImageList("IMG18{0:00}", new DateTime(2018, 10, 31, 18, 2, 21), 200, 20);

            listOfFakeImages.AddRange(secondImageList);

            var thirdImageList = await GenerateImageList("IMG19{0:00}", new DateTime(2018, 10, 31, 18, 4, 42), 200, 10);
